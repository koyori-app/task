...
コミットメッセージや PR タイトルに `#N`（プロジェクト内連番）を書くだけで自動リンクされ、`Closes #N` でタスクが自動クローズされる。
...
PR タイトル・PR 本文・コミットメッセージ・ブランチ名から `#N` を正規表現でパースする。

| パターン | 例 | 動作 |
|---------|-----|------|
| `#N`（タイトル / メッセージ） | `fix: ログインバグ修正 #42` | タスク #42 にリンク追加 |
| 複数 `#N` | `feat: OAuth #42 #43` | タスク #42 と #43 両方にリンク追加 |
| ブランチ名 | `feat/task-42-oauth` | タスク #42 にブランチリンク追加 |
| クローズキーワード + `#N` | `Closes #42` / `Fixes #42` / `Resolves #42` | PR マージ時にタスク #42 を自動クローズ |

**クローズキーワード:** `Closes` / `Close` / `Fixes` / `Fix` / `Resolves` / `Resolve`（大小文字不問）  
PR タイトルおよび PR 本文の両方が対象。

### seq_id の解決

`#N` は **そのリポジトリと紐付いているプロジェクト** 内の `seq_id=N` のタスクを探す。  
見つからない場合はリンク追加をスキップ（エラーにしない）。
...
```markdown
---
🔗 Linked Task: [#42 OAuth 対応を実装する](https://app.example.com/projects/.../tasks/42)
```
...
│                                             │
│ 使い方:                                     │
│   コミットや PR タイトルに #42 を含めると   │
│   タスク #42 に自動でリンクされます。       │
│   Closes #42 と書くと PR マージ時に         │
│   タスクが自動的に完了になります。          │
└──────────────────────────────────────────────┘
```

...
pub struct Model {
    ...
    pub is_default_done: bool, // 「完了にする」操作の移動先（完了のうち 1 つのみ）
...
| `is_default_done` | BOOLEAN | NOT NULL DEFAULT false |
| `created_at` | TIMESTAMPTZ | NOT NULL DEFAULT now() |
| — | — | UNIQUE(project_id, name) |

> プロジェクト作成時に `Backlog`（is_default=true）/ `In Progress` / `In Review` / `Done`（is_done_state=true）を自動挿入する。

完了ステータスは複数持てる（`Done` と `No Planning` のように）。そのうち 1 つに `is_default_done`
を付け、CLI の `tasks complete`・タスク詳細の完了ボタン・GitHub Issue の close 同期はここへ移す。
印が無いプロジェクトでは並び順で最初の完了ステータスを使う。

- `is_default_done` は完了ステータスにしか付けられず、付け替えは他から自動で外れる
- 完了ステータスが 1 つも無い状態にはできない（最後の 1 つは `is_done_state` を外せず、削除もできない）
- 完了を外したステータスにあるタスクは `completed_at` が消える（完了へ移したタスクには入る）
...
| `priority` | string[]? | |
| `assignee_id` | UUID? | |
| `label_id` | UUID? | 指定ラベルが付与されたタスクのみ |
| `milestone_id` | UUID? | |
...
| 操作 | 権限 |
|------|------|
| タスク閲覧 / 作成 / 更新 / アーカイブ | プロジェクトに入れる人 or テナントオーナー |
| タスク削除（ソフト） | 作成者 or テナントオーナー |
| ステータス / ラベル管理 | プロジェクトに入れる人 or テナントオーナー |
| マイルストーン管理 | プロジェクトに入れる人 or テナントオーナー |

「プロジェクトに入れる人」は、テナントの所属を継承した人と、そのプロジェクトに明示 ACE（`project_members` の行）を持つ人である。
プロジェクトにメンバーを 1 人も指定していなければテナントメンバー全員が入れ、指定があれば明示 ACE を持つ人だけが入れる。
明示 ACE はテナント所属と独立に残るので、テナントに行が無い人でも明示 ACE のあるプロジェクトには客分として入れる（テナント全体の口は開かない）。

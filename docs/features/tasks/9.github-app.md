...
> `installation_gone`、一時障害なら installation を控えたうえで `github_unavailable`
...
取り込み・webhook による新規作成は「GitHub Issueからタスクを作成しました」、
既存タスクへの変更反映は「GitHub Issueからタスクを同期しました」とアクティビティに表示し、
リポジトリ名と Issue 番号を併記する。システム操作としてタスク更新と同時に記録し、
同じ内容や古い通知では追加しない（[イベント定義](/features/tasks/collaboration)）。

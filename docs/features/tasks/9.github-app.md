...
200 OK
{
  "url": "https://github.com/apps/{APP_NAME}/installations/new?state=xxx"
}
```

> フロントエンドが `fetch` で URL を取得してから `window.location` へ遷移するため、HTTP リダイレクト（`202 + Location`）ではなく JSON で返す。
...
```text
API 呼び出し前（PR #9b で GitHub API 呼び出し時に実装予定）:
  token_expires_at < now() + 5min → GitHub API で再取得 → DB 更新 → 使用
  それ以外 → DB の暗号化済みトークンを復号して使用
```

> Wave 0 ではトークン再取得ヘルパーは未実装。コールバック時に取得したトークンを DB に保存し、有効期限カラム（`token_expires_at`）のみ保持する。
...
**`installation_id` 重複時の挙動:**

`installation_id` に UNIQUE 制約は設けない。同一 GitHub App インストールを複数プロジェクトが共有できる設計とする。
Webhook 受信時は `installation_id`（および payload に `repository` が含まれる場合は `repo_owner` + `repo_name`）で一致する全 `github_integrations` 行を検索し、各行ごとに Apalis ジョブを enqueue する。
重複排除（`delivery_id` + `integration_id` ベースの冪等化）は PR #9b で追加予定。
...
| `DELETE` | `/v1/tenants/{tid}/projects/{pid}/github/integration` | 連携解除（GitHub 側の App uninstall API を呼び出してから DB 行を削除） |

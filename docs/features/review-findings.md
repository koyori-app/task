...
- **`write:review` は `read:review` を包含する**（起票した本人が自分の結果を読めないと
  ...
  ここで明示する（project 系の非包含はのちに欠落と判じ、全対を包含へ揃えた。
  apps/backend/docs/personal-access-tokens-authz.md の「含意の規則」）
...
- 状態遷移後も要約更新ジョブを投入する
- 起票と状態遷移は in-app 通知を作る（起票・遷移と同じトランザクション）。受信者の規則は
  [通知・ウォッチャー](/features/tasks/notifications) §3.1
...
- 読み取りの PR 選択は `repo=owner/name` + `pr=N` に加えて `host` / `host_url` または
  `pull_request_id` を受け付ける。`pull_request_id` は候補探索をせず対象を確定するが、併記した
...
- ホスト移行時の履歴選択: GitHub と Forgejo に同じ `org/app` の PR #10 があるとき、
  ...
  候補が 1 件の repo-only 指定は成功する。`--pull-request-id X` / `pull_request_id=X` と併記した
  `repo` / `host` / `host_url` のいずれかが X の行と一致しなければ API は 400、CLI は非 0 になる
...
- 2026-09-09: `pull_request_id` は候補探索を省略するが、併記された `repo` / `host` / `host_url` は
  対象行との一致を検証し、不一致を API 400 / CLI 非 0 で拒否する（レビュー指摘: Git ホスティング↔
  タスク連携仕様では不一致を拒否する一方、本仕様は他の指定を無視するように読めた）
...
- 2026-09-22: ラウンドの起票と指摘の状態遷移を in-app 通知にする。宛先は同じ PR の関係者
  （ラウンドの作成者・`fixed_by`・PR の作者）で、`review_round_any` を設定に入れた利用者は
  ラウンドの起票だけを追加で受け取る。購読の既定は OFF（既定にすると、PR を見ていない

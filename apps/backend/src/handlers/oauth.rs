...
use crate::utils::oauth::client::{TokenResponse, exchange_code, fetch_user_info};
use crate::utils::oauth::crypto::encrypt_token;
use crate::utils::oauth::pkce::{generate_pkce_pair, generate_state};
use crate::utils::oauth::provider::{
...
fn resolve_disconnect_provider(
    settings: &crate::utils::oauth::OAuthSettings,
    provider: &str,
) -> Result<String, OAuthError> {
    if provider == "oidc" {
        return settings

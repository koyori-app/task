...
/// 通知が属するプロジェクトの要約。API パスと画面 URL の組み立てに使う。
#[derive(Serialize, Deserialize, ToSchema)]
...
pub struct NotificationItem {
    ...
    /// `project_id` から引いた要約。引けなければ `null`
    #[schema(nullable)]
    pub project: Option<NotificationProjectSummary>,
    #[schema(nullable)]
    pub task: Option<NotificationTaskSummary>,
    #[schema(value_type = serde_json::Value)]
    pub payload: serde_json::Value,
    /// この行を指すカーソル。catch-up では受け取った最新の行のこの値を控え、
    /// 次回 `after` に渡す（`next_cursor` は取り切ると `null` になるので控えに使えない）
    pub cursor: String,
...
pub struct NotificationListResponse {
    /// 未読の総数。`unread` / `kind` / `cursor` / `after` の指定に関係なく、見える通知全体で数える
...
/// 通知一覧のクエリ。
///
/// 並びは `created_at DESC, id DESC` のみ（未読優先はしない。既読化で順位が変わると
/// カーソルが成り立たない）。ただし `after` 指定時だけは古い順（`created_at ASC, id ASC`）で
/// 返す。新しい側から `limit` 件を切ると、`after` との間の行を引く手段が無くなるため。

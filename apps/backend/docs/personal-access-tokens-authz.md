...
**なぜ Admin まで通すのか**: Admin はメンバーの追加・変更ができる（`require_tenant_admin` が
守るメンバー管理の口）。発行だけをオーナーに閉じても、Admin は自分の別アカウントを
メンバーに足すなどの迂回が組めてしまい、境目としての実効が無い。閉じて締め出されるのは
迂回を選ばない正しい Admin の手だけである。

**なぜ主に限る必要が無いと判じたのか**: PAT は「作った本人として」動くトークンであり
（`user_id` は発行者に固定）、発行によって得られる力は発行者が既に持つ力を超えない。
リソース束縛（`tenant_id` / `allowed_project_ids`）とスコープの検証は発行後の認可で
掛かるため、発行の口を主に限っても守れる物が増えない。

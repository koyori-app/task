...
async fn custom_fields_integration_suite() {
    ...
    let fields_base = format!(
        "/v1/tenants/{}/projects/{}/custom-fields",
        tp.tenant_id, tp.project_id
    );
    let tasks_base = format!(
    ...
    let get = app
        .get_with_session(&format!("{tasks_base}/{task_id}"))
        .await;
    ...
    assert!(
        values
            .iter()
            .any(|v| v["field"]["id"] == number_id && v["value"] == "5")
    );
    assert!(
        values.iter().any(|v| v["field"]["id"] == select_id
            && v["value"] == "m"
            && v["display_value"] == "M")
    );
    assert!(
        values
            .iter()
            .any(|v| v["field"]["id"] == text_id && v["value"] == "hello")
    );
    assert!(
        values
            .iter()
            .any(|v| v["field"]["id"] == date_id && v["value"] == "2025-01-15")
    );
    assert!(
        values
            .iter()
            .any(|v| v["field"]["id"] == url_id && v["value"] == "https://example.com")
    );
    assert!(
        values
            .iter()
            .any(|v| v["field"]["id"] == checkbox_id && v["value"] == "true")
    );
    ...
    let delete = app
        .delete_with_session(&format!("{fields_base}/{text_id}"))
        .await;
    ...
    assert!(
        !values2.iter().any(|v| v["field"]["id"] == text_id),
        "削除済みフィールドは返らない"
    );
}

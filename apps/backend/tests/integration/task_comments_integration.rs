...
async fn login_as_tenant_member(
    app: &mut TestApp,
    project_id: uuid::Uuid,
) -> crate::common::TestUser {
    let member = app.insert_user(false, false).await;
    crate::common::ensure_tenant_member_for_project(&app.state.db, project_id, member.id).await;
...
async fn setup_task(
    ...
    crate::common::TestTenantProject,
    String,
    crate::common::TestUser,
) {

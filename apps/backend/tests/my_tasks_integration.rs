...
async fn create_status(app: &TestApp, tp: &TestTenantProject, name: &str, is_done: bool) -> Uuid {
    let path = format!("/v1/tenants/{}/projects/{}/statuses", tp.tenant_id, tp.project_id);
    let response = app.post_json_with_session(&path, serde_json::json!({
        "name": name, "color": "#336699",
        "position": if is_done { 2 } else { 1 },
        "is_default": name == "Todo", "is_done_state": is_done,
    })).await;
    assert_eq!(response.status(), StatusCode::CREATED);
    response.json::<serde_json::Value>().await.unwrap()["id"].as_str().unwrap().parse().unwrap()
}
...
async fn personal_project_is_idempotent() {
    ...
    let first: Uuid = app.get_with_session(&format!("{base}/personal-project")).await
        .json::<serde_json::Value>().await.unwrap()["id"].as_str().unwrap().parse().unwrap();
    let second: Uuid = app.get_with_session(&format!("{base}/personal-project")).await
        .json::<serde_json::Value>().await.unwrap()["id"].as_str().unwrap().parse().unwrap();
...
async fn quick_capture_and_list() {
    ...
    let (_user, tp) = setup(&mut app).await;
    let base = my_tasks_base(tp.tenant_id);
    assert_eq!(app.post_json_with_session(&format!("{base}/tasks"), serde_json::json!({"title": "Buy milk"})).await.status(), StatusCode::CREATED);
    let body: serde_json::Value = app.get_with_session(&format!("{base}/tasks?filter=all")).await.json().await.unwrap();
    assert!(body["tasks"].as_array().unwrap().iter().any(|t| t["title"] == "Buy milk"));
}
...
async fn list_projects_excludes_personal() {
    let mut app = TestApp::new().await;
    let (_user, tp) = setup(&mut app).await;
    app.get_with_session(&format!("{}/personal-project", my_tasks_base(tp.tenant_id))).await;
    let projects: Vec<serde_json::Value> = app.get_with_session(&format!("/v1/tenants/{}/projects", tp.tenant_id)).await.json().await.unwrap();
...
async fn filter_overdue() {
    ...
    let overdue = (Utc::now().date_naive() - Duration::days(2)).and_hms_opt(12,0,0).unwrap().and_utc();
    let path = format!("/v1/tenants/{}/projects/{}/tasks", tp.tenant_id, tp.project_id);
    ...
    let titles: Vec<String> = app.get_with_session(&format!("{}/tasks?filter=overdue", my_tasks_base(tp.tenant_id))).await
        .json::<serde_json::Value>().await.unwrap()["tasks"].as_array().unwrap()
        .iter().filter_map(|t| t["title"].as_str().map(str::to_string)).collect();
